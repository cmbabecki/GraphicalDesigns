...
function [eigenspaces, uniqueEigvals] = FindEigenspacesDminA(A)
...
[~, I] = sort(abs(eigvals - maxDeg),'descend');
eigvals = eigvals(I);
U = U(:, I);

%% separating the eigenspaces
uniqueEigvals = uniquetol(eigvals);
[~, I] = sort(abs(uniqueEigvals - maxDeg), 'descend');
