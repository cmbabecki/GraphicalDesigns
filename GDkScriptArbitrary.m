% A script to find an arbitrarily weighted k-design for each possible choice
% of k. Uncomment the graph adjacency matrix you wish to compute. Make sure
% you have working paths to Gurobi, YALMIP, and the adjacency matrix you are 
% interested in. The optimization solves a 1-norm relaxation of what is truly
% a 0-norm problem, so there is no guarantee of optimality. We note that the
% program typically finds positive weights, despite not being asked to do so. 
...
%% setup
assert(all(all(A == A')), 'A is not symmetric')
...
for k = 1: m - 1 %can never integrate all eigenspaces with a proper subset
    k
	[gdk_min_sub_size(k), gdk_opt(:,k), U_gdk_opt(:,k), runtime] = findGDkArbitrary(eigenspaces,k);
...
for j = 1:size(gdk_opt,2)
    efficacy(j) = sum(gdk_min_sub_size(:,j)) / (sum(multiplicities(1:j)));
end
...
% there is always an extremal design with arbitrary weights, so we do not
...
for i = 1:N
	...
	if nbhd(:,i) >= ones(n,1) %check is 1-neighborhood is graph
